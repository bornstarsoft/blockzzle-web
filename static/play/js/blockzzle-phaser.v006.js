(function (root, factory) {
  ...
  function getReturnGhostScale(ghostSize, trayPieceSize) {
    ...
    const trayRatio = trayPieceSize / ghostSize;
    return Math.max(0.58, Math.min(0.76, trayRatio * 0.86));
  }
  ...
  class BlockzzleScene extends SceneBase {
    ...
    playLineClearFeedback(result) {
      ...
      result.cells.forEach(([cellX, cellY], index) => {
        ...
        burst.setStrokeStyle(2, 0xffffff, 0.78);
        burst.setDepth(70);
    ...
    startDrag(slotIndex, pointer) {
      ...
      const ghostSize = Math.max(22, Math.floor(this.cellSize * 0.56));
      this.dragState = {
        slotIndex,
        piece,
