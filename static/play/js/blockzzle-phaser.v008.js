  ...
  class BlockzzleScene extends SceneBase {
    ...
    renderBoard() {
      ...
      for (let y = 0; y < 8; y += 1) {
        for (let x = 0; x < 8; x += 1) {
          ...
          if (isPreview) {
            const style = getPreviewCellStyle(
              this.dragState && this.dragState.piece,
              this.dragState && this.dragState.valid
            );
            fill = style.fill;
            alpha = style.alpha;
    ...
    playLineClearFeedback(result) {
      ...
      result.cells.forEach(([cellX, cellY], index) => {
        ...
        burst.setStrokeStyle(2, 0xffffff, 0.9);
        burst.setDepth(70);
    ...
    playPlacedPieceFeedback(result, piece) {
      ...
      result.placedCells.forEach(([cellX, cellY], index) => {
        ...
        settle.setStrokeStyle(2, 0xffffff, 0.38);
        settle.setScale(0.88);
        settle.setDepth(68);
