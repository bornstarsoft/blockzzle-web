(function (root, factory) {
  ...
  class BlockzzleScene extends SceneBase {
    ...
    addViewportListeners() {
      ...
      this.audioGestureHandler = () => this.primeAudioForGesture();
      ...
      window.addEventListener("pointerdown", this.audioGestureHandler, { passive: true });
      window.addEventListener("touchstart", this.audioGestureHandler, { passive: true });
      window.addEventListener("click", this.audioGestureHandler, { passive: true });
    ...
    removeViewportListeners() {
      ...
      if (this.audioGestureHandler) {
        window.removeEventListener("pointerdown", this.audioGestureHandler);
        window.removeEventListener("touchstart", this.audioGestureHandler);
        window.removeEventListener("click", this.audioGestureHandler);
      }
      if (window.visualViewport) {
    ...
    toggleSound() {
      ...
      this.updateSoundButton();
      if (this.soundEnabled) this.playSoundCue("restart");
    }

    primeAudioForGesture() {
      const state = this.audioContext ? this.audioContext.state : null;
      if (!shouldPrimeAudioOnGesture(this.soundEnabled, state)) return;
    ...
    playSoundCue(name) {
      ...
      try {
        if (context.state === "suspended" && typeof context.resume === "function") {
          context.resume()
            .then(() => {
              if (context.state === "running") this.playSoundCue(name);
            })
            .catch(() => {});
          return;
        }
