(function (root, factory) {
  ...
  class BlockzzleScene extends SceneBase {
    ...
    addViewportListeners() {
      ...
      this.audioGestureHandler = () => this.ensureAudioReady();
    ...
    ensureAudioReady() {
      ...
      try {
        // Saved Sound On may update the label on load, but iOS Safari still
        // requires a later user gesture before WebAudio can produce sound.
        this.playSilentUnlockTick(context);
        const finish = () => {
          this.playSilentUnlockTick(context);
    ...
    playSilentUnlockTick(context) {
      try {
        if (!context || typeof context.createBufferSource !== "function" || typeof context.createBuffer !== "function") return;
        const source = context.createBufferSource();
        const sampleRate = context.sampleRate || 44100;
        source.buffer = context.createBuffer(1, 1, sampleRate);
        if (typeof context.createGain === "function") {
          const gain = context.createGain();
          gain.gain.setValueAtTime(0.00001, context.currentTime || 0);
          source.connect(gain);
          gain.connect(context.destination);
        } else {
          source.connect(context.destination);
        }
        // The buffer is silent; it only unlocks the audio graph inside a gesture.
        source.start(0);
        if (typeof source.stop === "function") source.stop((context.currentTime || 0) + 0.02);
      } catch (error) {
