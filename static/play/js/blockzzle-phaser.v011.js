(function (root, factory) {
  ...
  const AUDIO_UNLOCK_CUE = { type: "unlock", frequency: 420, endFrequency: 420, durationMs: 45, volume: 0.001, wave: "sine" };
  ...
  function getAudioUnlockStatus(isSoundEnabled, audioState, audioUnlocked) {
    const hasReadyState = audioState === "running" || (audioUnlocked && !audioState);
    const ready = Boolean(isSoundEnabled && audioUnlocked && hasReadyState);
    return {
  ...
  class BlockzzleScene extends SceneBase {
    ...
    create() {
      ...
      this.audioUnlockPending = this.soundEnabled;
    ...
    addViewportListeners() {
      ...
      this.audioGestureHandler = () => this.ensureAudioUnlockedFromGesture("global-gesture");
      ...
      if (this.soundEnabled) this.addAudioUnlockListeners();
    ...
    restartGame() {
      this.ensureAudioUnlockedFromGesture("restart")
        .then((ready) => {
          if (ready) this.playSoundCue("restart");
        })
        .catch(() => {});
    ...
    loadSoundPreference() {
      try {
        return parseSoundPreference(window.localStorage.getItem(SOUND_STORAGE_KEY));
      } catch (error) {
        return false;
      }
    }
    ...
    saveSoundPreference() {
      try {
        window.localStorage.setItem(SOUND_STORAGE_KEY, String(this.soundEnabled));
      } catch (error) {
    ...
    updateSoundButton() {
      ...
      this.soundButton.setText(getSoundToggleLabel(this.soundEnabled));
      this.soundButton.setColor(this.soundEnabled ? "#07111f" : "#d6e2f4");
      this.soundButton.setBackgroundColor(this.soundEnabled ? "#facc15" : "#172238");
    }
    ...
    toggleSound() {
      ...
      this.ensureAudioUnlockedFromGesture("sound-toggle").then((ready) => {
        if (ready) this.playSoundCue("restart");
      }).catch(() => {});
    }

    primeAudioForGesture() {
      this.ensureAudioUnlockedFromGesture("phaser-pointer");
    }
    ...
    ensureAudioReady() {
      return this.ensureAudioUnlockedFromGesture("legacy");
    }

    ensureAudioUnlockedFromGesture(reason) {
      if (!this.soundEnabled) {
        this.audioUnlockPending = false;
      ...
      try {
        ...
        // Use the same oscillator/gain path as normal cues, just near-silent.
        this.playGestureUnlockTick(context, reason);
        const finish = () => {
          this.playGestureUnlockTick(context, reason);
          this.audioUnlocked = !context.state || context.state === "running";
          this.audioUnlockPromise = null;
    ...
    playGestureUnlockTick(context) {
      try {
        this.playTone(context, AUDIO_UNLOCK_CUE, 0);
      } catch (error) {
    ...
    playSoundCue(name) {
      ...
      if (!status.ready) {
        this.ensureAudioUnlockedFromGesture(`cue-${name}`)
          .then((ready) => {
    ...
    renderGameOverPanel() {
      ...
      panel.setDepth(90);
      const title = this.add.text(centerX, centerY - 100, "Game over", this.smallTextStyle(27, "#f8fbff", "800"));
      ...
      stats.setDepth(91);
      const button = this.add.text(centerX, centerY + 68, "Play again", this.smallTextStyle(16, "#07111f", "800"))
        .setOrigin(0.5, 0)
        .setPadding(28, 11)
        .setBackgroundColor("#8ee8d2")
        .setInteractive({ useHandCursor: true })
    ...
    handlePointerUp(pointer) {
      if (!this.dragState || this.dragState.returning) return;
      this.ensureAudioUnlockedFromGesture("placement-attempt");
