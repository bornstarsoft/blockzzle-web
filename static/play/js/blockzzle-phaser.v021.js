(function (root, factory) {
  ...
  const DAILY_DATE_STORAGE_KEY = "blockzzleDailyDateV1";
  const DAILY_BEST_SCORE_STORAGE_KEY = "blockzzleDailyBestScoreV1";
  const DAILY_BEST_LINES_STORAGE_KEY = "blockzzleDailyBestLinesV1";
  const DAILY_BEST_CLEAR_STORAGE_KEY = "blockzzleDailyBestClearV1";
  const DAILY_PLAYS_STORAGE_KEY = "blockzzleDailyPlaysV1";
  const DAILY_LAST_SCORE_STORAGE_KEY = "blockzzleDailyLastScoreV1";
  ...
  function hashSeed(input) {
    const text = String(input || "blockzzle");
    let hash = 2166136261;
  ...
  function createDailyRng(dateKey) {
    return createSeededRng(`blockzzle-daily:${dateKey || getTodayKey()}`);
  }

  function normalizeDailyStats(raw, todayKey) {
    const input = raw || {};
    const dateKey = todayKey || getTodayKey();
    const storedDate = input.dailyDate || "";
    const sameDate = storedDate === dateKey;
    return {
      dailyDate: dateKey,
      dailyBestScore: sameDate ? parseStoredStat(input.dailyBestScore) : 0,
      dailyBestLines: sameDate ? parseStoredStat(input.dailyBestLines) : 0,
      dailyBestClear: sameDate ? parseStoredStat(input.dailyBestClear) : 0,
      dailyPlays: sameDate ? parseStoredStat(input.dailyPlays) : 0,
      dailyLastScore: sameDate ? parseStoredStat(input.dailyLastScore) : 0,
    };
  }

  function completeDailyStats(stats, score, lines, todayKey, bestClearLines) {
    const dateKey = todayKey || getTodayKey();
    const current = normalizeDailyStats(stats, dateKey);
    const completedScore = Math.max(0, Number.parseInt(score || 0, 10) || 0);
    const completedLines = Math.max(0, Number.parseInt(lines || 0, 10) || 0);
    const completedBestClear = Math.max(0, Number.parseInt(bestClearLines || 0, 10) || 0);
    return {
      dailyDate: dateKey,
      dailyBestScore: Math.max(current.dailyBestScore, completedScore),
      dailyBestLines: Math.max(current.dailyBestLines, completedLines),
      dailyBestClear: Math.max(current.dailyBestClear, completedBestClear),
      dailyPlays: current.dailyPlays + 1,
      dailyLastScore: completedScore,
    };
  }
  ...
  function getDailyPieceSequence(dateKey, shapes, count) {
    const game = new Game({
      rng: createDailyRng(dateKey),
      shapes: shapes && shapes.length ? shapes : SHAPES,
      storage: null,
    });
    const pieces = game.tray.slice();
    while (pieces.length < (count || TRAY_SIZE)) {
      pieces.push(game.createRandomPiece());
    }
    return pieces.slice(0, count || TRAY_SIZE).map((piece) => ({
      id: piece.id,
  ...
  class BlockzzleScene extends SceneBase {
    constructor() {
      ...
      this.dailyDateKey = getTodayKey();
      this.repeatStats = normalizeRepeatStats({}, getTodayKey());
      this.dailyStats = normalizeDailyStats({}, this.dailyDateKey);
    ...
    buildStaticUi() {
      ...
      this.classicModeButton = this.add.text(0, 0, "Classic", this.smallTextStyle(12, "#d6e2f4", "800"))
        .setPadding(9, 6)
        .setInteractive({ useHandCursor: true })
        .on("pointerdown", () => this.switchMode("classic"));
      this.dailyModeButton = this.add.text(0, 0, "Daily", this.smallTextStyle(12, "#d6e2f4", "800"))
        .setPadding(9, 6)
        .setInteractive({ useHandCursor: true })
        .on("pointerdown", () => this.switchMode("daily"));
    ...
    updateModeButtons() {
      if (!this.classicModeButton || !this.dailyModeButton) return;
      const activeColor = "#8ee8d2";
      const inactiveColor = "#172238";
      const activeText = "#07111f";
      const inactiveText = "#d6e2f4";
      this.classicModeButton
        .setBackgroundColor(this.gameMode === "classic" ? activeColor : inactiveColor)
        .setColor(this.gameMode === "classic" ? activeText : inactiveText);
      this.dailyModeButton
        .setBackgroundColor(this.gameMode === "daily" ? activeColor : inactiveColor)
        .setColor(this.gameMode === "daily" ? activeText : inactiveText);
    }
    ...
    createGameModel() {
      if (this.gameMode === "daily") {
        this.dailyDateKey = getTodayKey();
        return new Game({
          rng: createDailyRng(this.dailyDateKey),
          storage: null,
        });
      }
      return new Game();
    ...
    switchMode(mode) {
      ...
      if (this.gameModel && this.gameModel.gameOver) this.recordCompletedGameStats();
      if (this.gameMode === "classic" && this.gameModel) this.gameModel.saveHighScore();
      ...
      this.destroyFeedbackItems();
      this.destroyGameOverPanel();
      this.statusText.setText(mode === "daily" ? "Same pieces today. Beat your best." : "");
      this.updateModeButtons();
      this.render();
    }
    ...
    loadDailyStats() {
      ...
      try {
        return normalizeDailyStats({
          dailyDate: window.localStorage.getItem(DAILY_DATE_STORAGE_KEY),
          dailyBestScore: window.localStorage.getItem(DAILY_BEST_SCORE_STORAGE_KEY),
          dailyBestLines: window.localStorage.getItem(DAILY_BEST_LINES_STORAGE_KEY),
          dailyBestClear: window.localStorage.getItem(DAILY_BEST_CLEAR_STORAGE_KEY),
          dailyPlays: window.localStorage.getItem(DAILY_PLAYS_STORAGE_KEY),
          dailyLastScore: window.localStorage.getItem(DAILY_LAST_SCORE_STORAGE_KEY),
        }, todayKey);
      } catch (error) {
        return normalizeDailyStats({}, todayKey);
      }
    }

    saveDailyStats(stats) {
      try {
        window.localStorage.setItem(DAILY_DATE_STORAGE_KEY, stats.dailyDate);
        window.localStorage.setItem(DAILY_BEST_SCORE_STORAGE_KEY, String(stats.dailyBestScore));
        window.localStorage.setItem(DAILY_BEST_LINES_STORAGE_KEY, String(stats.dailyBestLines));
        window.localStorage.setItem(DAILY_BEST_CLEAR_STORAGE_KEY, String(stats.dailyBestClear));
        window.localStorage.setItem(DAILY_PLAYS_STORAGE_KEY, String(stats.dailyPlays));
        window.localStorage.setItem(DAILY_LAST_SCORE_STORAGE_KEY, String(stats.dailyLastScore));
      } catch (error) {
        // Daily Challenge records are local-only motivation; play must continue.
      }
    }
    ...
    recordCompletedGameStats() {
      ...
      const todayKey = getTodayKey();
      if (this.gameMode === "daily") {
        this.dailyStats = completeDailyStats(
          this.loadDailyStats(),
    ...
    resizeLayout() {
      ...
      this.updateSoundButton();
      this.classicModeButton.setFontSize(tiny ? 10 : compact ? 11 : 12);
      this.dailyModeButton.setFontSize(tiny ? 10 : compact ? 11 : 12);
      this.classicModeButton.setPadding(compact ? 8 : 9, compact ? 5 : 6);
      this.dailyModeButton.setPadding(compact ? 8 : 9, compact ? 5 : 6);
      this.updateModeButtons();
      ...
      const modeY = topY + (tiny ? 46 : compact ? 50 : 76);
      this.classicModeButton.setPosition(margin, modeY);
      this.dailyModeButton.setPosition(this.classicModeButton.x + this.classicModeButton.width + 5, modeY);
    ...
    render() {
      ...
      if (this.gameMode === "daily") {
        this.dailyStats = this.loadDailyStats();
      } else if (!this.repeatStats) {
        this.repeatStats = this.loadRepeatStats();
      }
      this.scoreText.setText(`Score ${this.gameModel.score}`);
      this.bestText.setText(this.gameMode === "daily"
        ? `Today ${this.dailyStats.dailyBestScore}`
        : `Best ${this.gameModel.highScore}`);
      this.todayBestText.setText(this.gameMode === "daily"
        ? `Plays ${this.dailyStats.dailyPlays}`
        : `Today ${this.repeatStats.todayBest}`);
      this.linesText.setText(`Lines ${this.gameModel.linesCleared}`);
      this.nextGoalText.setText(this.gameMode === "daily"
        ? "Daily Challenge"
        : getScoreTierGoalText(this.gameModel.score));
      this.renderBoard();
      this.renderTray();
      this.renderGameOverPanel();
      if (this.gameModel.gameOver) {
        this.statusText.setText(this.gameMode === "daily" ? "New challenge tomorrow." : "No more moves. Play again?");
      }
    }
    ...
    renderGameOverPanel() {
      ...
      const dailyStats = this.dailyStats || this.loadDailyStats();
      ...
      const isDailyMode = this.gameMode === "daily";
      const statsLines = isDailyMode
        ? [
          `Score ${this.gameModel.score}    Today Best ${dailyStats.dailyBestScore}`,
          `Daily Plays ${dailyStats.dailyPlays}    Lines ${this.gameModel.linesCleared}`,
          `Best Clear: ${dailyStats.dailyBestClear} ${dailyStats.dailyBestClear === 1 ? "Line" : "Lines"}`,
          `Last Score ${dailyStats.dailyLastScore}`,
      ...
      panel.setStrokeStyle(2, 0xfacc15, 0.92);
      panel.setDepth(90);
      const title = this.add.text(centerX, centerY - 132, isDailyMode ? "Daily Challenge" : "Game over", this.smallTextStyle(26, "#f8fbff", "800"));
      title.setOrigin(0.5, 0);
      title.setDepth(91);
      const reason = this.add.text(
        centerX,
        centerY - 92,
        isDailyMode ? "Same pieces today. Beat your best." : "Beat your best tier.",
  ...
  return {
    BlockzzleCore: {
      ...
      DAILY_BEST_SCORE_STORAGE_KEY,
      DAILY_BEST_LINES_STORAGE_KEY,
      DAILY_BEST_CLEAR_STORAGE_KEY,
      DAILY_PLAYS_STORAGE_KEY,
      DAILY_LAST_SCORE_STORAGE_KEY,
      SOUND_CUES,
      AUDIO_UNLOCK_CUE,
