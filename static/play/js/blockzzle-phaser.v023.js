(function (root, factory) {
  ...
  const LEADERBOARD_BLOCKED_NICKNAMES = ["admin", "moderator", "support", "badword"];
  ...
  function validateLeaderboardSubmission(input) {
    ...
    return {
      ...
      entry: {
        ...
        client_version: String(body.client_version || CLIENT_VERSION).trim().slice(0, 24) || CLIENT_VERSION,
        browser_player_id: String(body.browser_player_id || "").trim().slice(0, 80),
      },
    };
  }
  ...
  class BlockzzleScene extends SceneBase {
    ...
    async submitLeaderboardScore() {
      ...
      try {
        ...
        if (!response.ok || !data || !data.ok) {
          this.leaderboardSubmitStatus = (data && data.message) || this.leaderboardUnavailableMessage(response);
          return;
        }
    ...
    renderGameOverPanel() {
      ...
      const leaderboardLines = this.getLeaderboardPreviewLines();
      const submitLabel = this.leaderboardSubmitting
        ? "Submitting..."
        : this.leaderboardSubmitted ? "Submitted" : "Submit Score";
      ...
      panel.setStrokeStyle(2, 0xfacc15, 0.92);
      panel.setDepth(90);
      const title = this.add.text(centerX, centerY - 178, "Game over", this.smallTextStyle(24, "#f8fbff", "800"));
      title.setOrigin(0.5, 0);
      title.setDepth(91);
      ...
      stats.setDepth(91);
      const divider = this.add.rectangle(centerX, centerY + 6, panelWidth - 44, 1, 0x34445f, 0.88);
      divider.setDepth(91);
      ...
      const submitButton = this.add.text(centerX - 86, centerY + 54, submitLabel, this.smallTextStyle(12, "#07111f", "800"))
        .setOrigin(0.5, 0)
        .setPadding(12, 7)
        .setBackgroundColor(this.leaderboardSubmitted ? "#a7f3d0" : "#facc15")
        .setInteractive({ useHandCursor: true })
        .on("pointerdown", () => this.submitLeaderboardScore());
      submitButton.setDepth(91);
      const todayButton = this.add.text(centerX + 20, centerY + 54, "Today", this.smallTextStyle(11, this.leaderboardScope === "today" ? "#07111f" : "#d6e2f4", "800"))
        .setOrigin(0.5, 0)
        .setPadding(10, 7)
      ...
      const allTimeButton = this.add.text(centerX + 92, centerY + 54, "All-Time", this.smallTextStyle(11, this.leaderboardScope === "alltime" ? "#07111f" : "#d6e2f4", "800"))
        .setOrigin(0.5, 0)
        .setPadding(10, 7)
      ...
      const submitStatus = this.add.text(
        centerX,
        centerY + 123,
        this.leaderboardSubmitStatus || "Your local stats are saved even if the board is unavailable.",
      ...
      const button = this.add.text(centerX, centerY + 152, "Play Again", this.smallTextStyle(15, "#07111f", "800"))
        .setOrigin(0.5, 0)
        .setPadding(26, 10)
        .setBackgroundColor("#8ee8d2")
        .setInteractive({ useHandCursor: true })
